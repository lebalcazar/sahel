...
cdr_hist <- read_rds("data/rds/pronostico_poly_mensual.rds")  %>% 
 dplyr::select(-pred_poly) %>% 
  filter(lubridate::year(date) >= 1991) # nueva normal según PRESAO 2021
...
cdr_test <- cdr_hist |> 
  filter(
    ...
    lubridate::year(date) >= 1991) # nueva normal según PRESAO 2021s
...
ggplot(cdr_pred21) +
  ...
  geom_line(aes(y = normal, color = "Normal 1991-2020")) +
